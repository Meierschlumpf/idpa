...
export const PlanVacationItem = ({
	start,
	end,
	name,
	isNext,
	isCurrent,
}: PlanVacationItemProps) => {
	const {
		colors,
		fn: { rgba },
	} = useMantineTheme();
	...
	return (
		<Card
			...
			style={{
				...
				borderColor:
					isNext || isCurrent ? colors.teal[4] : rgba(colors.teal[4], 0.2),
				overflow: 'visible',
			}}
		>
			{isNext && (
			...
			<Title order={3}>{`${name}ferien`}</Title>
			<Text>{`${formatDate(start)} bis ${formatDate(end)} (${weekCount} Woche${
				weekCount !== 1 ? 'n' : ''
			})`}</Text>
		</Card>
	);
...
const getUntilEnd = (date: Date) => {
	...
	return days === 0
		...
		: days <= 7
		? `Enden in ${days} Tagen`
		: `Enden in ${Math.floor(days / 7)} Wochen`;
};
...
const getUntilStart = (date: Date) => {
	...
	const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
	const days = Math.floor(
		(date.getTime() - today.getTime()) / (1000 * 60 * 60 * 24) - 2,
	);
	return days === 0
		? 'Beginnen Heute'
		: days === 1
		? 'Beginnen Morgen'
		: days <= 7
		? `Beginnen in ${days} Tagen`
		: `Beginnen in ${Math.floor(days / 7)} Wochen`;
};
