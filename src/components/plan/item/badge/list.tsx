...
export const BadgeList = ({ handleAddition, handleRemoval, handleSwitch, mode, activeBadges }: Props) => {
	const { data: badges } = trpc.plan.getBadges.useQuery();
	const activeBadgeList = useMemo(() => activeBadges.map(ab => ({
		...ab,
	...
	return ((activeBadgeList?.length ?? 0) >= 1 || mode === 'write') ? (
		<Group>
			{activeBadgeList?.map(b => <PlanItemBadge key={b.id} variant={b.name as BadgeType} mode={mode} evaluated={b.evaluated} onRemove={() => handleRemoval(b.id)} onSwitch={() => handleSwitch(b.id, !b.evaluated)} />)}
			{mode === 'write' && (badges?.length ?? 0) > activeBadges.length && <Menu withinPortal>
				<Menu.Target>
				...
				<Menu.Dropdown>
					{badges?.filter(x => !activeBadges.some(y => y.id === x.id)).map(b => <Menu.Item key={b.id} onClick={() => handleAddition(b.id)}>{badgeNames[b.name as keyof typeof badgeNames]}</Menu.Item>)}
				</Menu.Dropdown>
			</Menu>}
		</Group>
	) : null;
