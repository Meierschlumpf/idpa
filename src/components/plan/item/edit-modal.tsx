...
export const PlanItemEditModal = ({ item, opened, closeModal }: Props) => {
	...
	const { mutateAsync } = trpc.plan.update.useMutation();
	...
	const handleSwitch = async (id: string, evaluated: boolean) => {
		...
		if (badges[index])
			badges[index]!.evaluated = evaluated;
	...
	return <Modal size="xl" title={<Title order={3}>Lektion bearbeiten</Title>} opened={opened} onClose={onClose}>
		<Stack>
			<Stack spacing={4}>
				<Text weight={500} size='sm'>Vorschau</Text>
				<PlanItem item={{
					id: item.id, date: item.date, title: form.values.title, description: form.values.description, badges: form.values.badges?.map(x => ({
						...x,
			...
			<form onSubmit={form.onSubmit(handleSubmit)}>
				...
				<Stack spacing={4}>
					<Text weight={500} size='sm'>Badges</Text>
				...
				<Group position="right">
					...
					<Button variant="light" type="submit">Speichern</Button>
				</Group>
			</form>
