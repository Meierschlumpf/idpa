...
interface HomeworkCreateModalProps extends BaseModalProps {
  planItemId: string;
  planId: string;
}

export const HomeworkCreateModal = ({ opened, closeModal, planId, planItemId }: HomeworkCreateModalProps) => {
  const utils = trpc.useContext();
  const { data: materials } = trpc.referenceMaterial.getByPlanId.useQuery({ planId });
  const { mutateAsync } = trpc.homework.create.useMutation();
  const onClose = () => {
    closeModal();
    form.reset();
  };
      ...
      },
      {
        onSuccess() {
          showSuccessNotification({
            ...
            message: 'Die Hausaufgabe wurde erfolgreich hinzugefügt!',
          });
  ...
  return (
    <Modal size="lg" opened={opened} onClose={onClose} title={<Title order={3}>Hausaufgabe hinzufügen</Title>}>
